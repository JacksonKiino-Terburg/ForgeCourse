...
import net.berlitz.mccourse.item.ModArmorMaterials;
...
public class ModArmorItem extends ArmorItem {
    //SET BONUSES only for custom armor

    private static final Map<ArmorMaterial, MobEffectInstance> MATERIAL_TO_EFFECT_MAP =
            (new ImmutableMap.Builder<ArmorMaterial, MobEffectInstance>())
                    .put(ModArmorMaterials.ALEXANDRITE, new MobEffectInstance(MobEffects.JUMP, 10, 1))
                    .build();
    ...
    private void addEffectToPlayer(Player player, MobEffectInstance mapEffect) {
        ...
        if(!hasPlayerEffect) {
            //must add from .get rather than explicitly
