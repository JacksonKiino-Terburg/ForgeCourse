package net.berlitz.mccourse.item.custom;
...
import net.berlitz.mccourse.util.InventoryUtil;
import net.berlitz.mccourse.util.ModTags;
...
public class MetalDetectorItem extends Item {
    ...
    public InteractionResult useOn(UseOnContext pContext) {
        if(!pContext.getLevel().isClientSide()) {
            ...
            if(!foundBlock){
                outputNoValueableCoordinates(player);
            }
        }
    ...
    private void outputNoValueableCoordinates(Player player) {
        player.sendSystemMessage(Component.translatable("item.mccourse.metal_detector.no_valuables"));
    }

    private void outputValueableCoordinates(BlockPos below, Player player, Block block) {
        player.sendSystemMessage(Component.literal("Valuable Found: " + I18n.get(block.getDescriptionId()) + " at ("
            + " at (" + below.getX() + ", " + below.getY() + ")"));
    }
